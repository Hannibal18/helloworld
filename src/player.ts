...
export function onAttackBroadcast(p: LocalPlayer, atk: AttackPayload, ctx: UpdateCtx, mode: 'pk' | 'zombie' | 'lobby' = 'pk'): void {
  if (p.dead) return;
  if (atk.id === p.id) return; // 자기 메시지는 self:false 라 안 옴 — 방어용
  if (ctx.now < p.iFrameUntil) return;
  if (mode !== 'pk') return; // 좀비/광장 = PvP 비활성. PK 모드에서만 데미지.
